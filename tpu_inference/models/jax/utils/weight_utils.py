...
def assign_and_shard_param(jax_param: nnx.Param,
    ...
    try:
        ...
        logger.error(f"Failed to shard/assign weight '{param_name}': {e}")
        raise RuntimeError(
            ...
            if not hasattr(param, "weight_loader"):
                ...
                if any(substr in name for substr in ["q_proj.weight"]):
                    ...
                    if len(param.value.shape) == 3:
                        ...
                        reshape_dims = (D, N, H)
                        permute_dims = (1, 2, 0)
                elif any(substr in name for substr in ["down_proj.weight", "gate_proj.weight", "up_proj.weight", "kv_a_proj_with_mqa.weight"]):
                    permute_dims = (0, 1)
